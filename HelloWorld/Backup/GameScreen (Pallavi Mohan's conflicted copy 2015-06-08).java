...
public class GameScreen implements Screen {
	  final Drop game;
		...
		Texture blockImage,wand,star, submit, bg, unavail, selectedblk,wordblk,wrongblk,b1,b2,b3, progress;
		TextureRegion pbar;
		OrthographicCamera camera;
		Rectangle bucket;
		Array<Rectangle> raindrops;
					...
					{	
						if(selected[3-xi][xj]==1)
							...
							//game.batch.draw(selectedblk, spx+xx, spy+yy);
								...
								if(shade[3-xi][xj]==0||shade[3-xi][xj]==3)
								{
									game.batch.draw(b1, spx+xx, spy+yy);
								}
								else if(shade[3-xi][xj]==1||shade[3-xi][xj]==4)
								{
									game.batch.draw(b2, spx+xx, spy+yy);
								}
								else
								{
									game.batch.draw(b3, spx+xx, spy+yy);
								}
		...
		/*		if(temp1==0)
			...
			//	game.font.draw(game.batch, "" + shade[i][j], (j*128)+10+spx, ((4-i)*128)+spy-40);
				}
			}
			...
			//if( Gdx.input.justTouched()) 
			{
				...
				else if (grid.contains(touchPos.x, touchPos.y))
				{
		...
		public void updateProgressBar(int time)
		{
			...
			progressLength=(pbar.getRegionWidth())*(time/150);
			if(progressLength>pbar.getRegionWidth()) progressLength=pbar.getRegionWidth();
			//barLength = new TextureRegion(pbar, 0, 0, progressLength, pbar.getRegionHeight());
			barLength = new TextureRegion(pbar, 0, 0, pbar.getRegionWidth(), pbar.getRegionHeight());
			game.batch.draw(barLength,28,1206);
